...
module.exports = {
    ...
    callback: async ({ msg, client, fullArgs }) => {
        let { data } = await axios.get('https://api.lolhuman.xyz/api/stalkig/{username}?apikey={apikey}'.format({ username: fullArgs, apikey })).catch(() => { return msg.reply('Server dalam perbaikkan') })
