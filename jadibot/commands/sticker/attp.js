...
const { writeExifVid } = require('@jadibot/libs/converter/exif')
...
module.exports = {
    ...
    callback: async ({ msg, client, message, fullArgs, shannMe }) => {
        const { packname, author } = require(`@jadibot/config-${shannMe}.json`)
        ...
        canvasGif(file, (ctx, width, height, totalFrames, currentFrame) => {
            ...
            .then(async (buffer) => {
                let buff = await writeExifVid(buffer, { packname, author })
                client.sendMessage(msg.from, { sticker: { url: buff }, packname, author }, { quoted: message })
            })
