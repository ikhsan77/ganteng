...
module.exports = {
    ...
    callback: async ({ msg, client, message }) => {
        ...
        if (msg.typeCheck.isQuotedSticker) {
            ...
            client.sendMessage(msg.from, { video: { url: result }, caption: '_Done by SHANNBot_', gifPlayback: true }, { quoted: message }).catch(() => { return msg.reply('Server dalam perbaikkan') })
            await fs.unlinkSync('database/src/shanndev.webp')
        } else return msg.reply('Send/reply video dengan caption #togif')
    }
}